...
SELECT eno, ename, TRUNC(sal*12+NVL(comm,0)) 연봉
FROM emp;
...
-- 해당 단위를 기준으로 하위를 반올림
--Docker에 설치한 Oracle Server가 표준시라서 서울보다 9시간 느리다
SELECT sysdate,
ROUND(sysdate, 'YY') 년,
ROUND(sysdate, 'MM') 월,
ROUND(sysdate, 'DD') 일
...
10일
10-1 = 9 + 1
SELECT ename, TRUNC(sysdate, 'DD') - TRUNC(hdate, 'DD')+1 "일한 일수"
FROM emp
WHERE ename='김연아';
...
SELECT eno, ename, hdate 입사일, hdate+99 "100일", ADD_MONTHS (hdate, 120) "10년째 되는 날"
FROM emp
WHERE dno='20';
