...
FROM emp, salgrade
WHERE job='개발' AND sal BETWEEN losal AND hisal;
...
SELECT dname, grade, sal, eno, ename, job
FROM emp
...
SELECT e1.mgr "매니저 사번", e2.ename "매니저명", e1.eno "사원번호", e1.ename "사원명"
FROM emp e1
...
; (+)기호로 하는 외부조인은 둘중에 한쪽에만 사용가능하다
  ...
  a. (+) : POS부서는 존재, POS에 부서원이 없을 때
           dept.dno=emp.dno(+)
  b. 부서원이 없는 POS부서 존재
     피카츄 신입사원의 부서가 배정되기 전
     이럴 경우는 FULL JOIN을 사용해야 한다.
     
5) 피카츄 사원을 추가해보자

-- 그냥 검색했을 때는 POS도 안나오고, 도람푸/피카츄도 안나온다.
-- 양쪽 조건이 일치하지 않기 때문에
-- POS 부서에는 해당 부서 사번을 가진 사원이 null,
-- 도람푸/피카츄은 소속 부서 번호가 null
...
SELECT dept.dno, dname, eno, ename
FROM emp
RIGHT OUTER JOIN dept ON dept.dno=emp.dno
ORDER BY dno;
...
SELECT dept.dno, dname, eno, ename
FROM emp
LEFT OUTER JOIN dept ON dept.dno=emp.dno
ORDER BY dno;
...
SELECT dept.dno, dname, eno, ename
FROM emp
FULL JOIN dept ON dept.dno=emp.dno
ORDER BY dno;
