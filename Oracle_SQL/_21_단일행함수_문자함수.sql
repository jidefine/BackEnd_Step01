...
SELECT TRIM('남' from '남기남'),          --문자열 '남기남'에서 '남'을 제거
        TRIM(leading '남' from '남기남'), --문자열 '남기남'에서 왼쪽(leading)부터 '남'을 제거
        TRIM(trailing '남' from '남기남'),--문자열 '남기남'에서 오른쪽(trailing)부터 '남'을 제거
        TRIM('남' from '남남남기남남남')  --문자열 '남남남기남남남'에서 양쪽 끝에서 '남'을 제거
 FROM dual;

7) 이름과 급여를 각각 10컬럼으로 검색한다
--RPAD 오른쪽 패딩 / LPAD 왼쪽 패딩
SELECT RPAD(ename, 10, '*'), LPAD(sal, 10, '*')
FROM emp;

8) 부서명의 마지막 글자를 제외하고 검색한다
--SUBSTR(문자열, 1번째 글자부터, '문자열 길이-마지막 문자'만큼) 새로운 열의 별명
