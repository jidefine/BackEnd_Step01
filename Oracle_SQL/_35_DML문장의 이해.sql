...
SELECT * FROM emp; --되살아남
SELECT * FROM dept; --되살아남
...
SELECT * FROM emp; --삭제된 상태로 저장
SELECT * FROM dept; --삭제된 상태로 저장
...
INSERT INTO emp(eno, ename, job, mgr, hdate, sal, comm, dno)
    VALUES('1905', '제니', '백수', null, sysdate, 구천, 9999, '40');


-- '9000' varchar2타입이 9000 number 타입으로 자동 형 변환
-- 컬럼 값에 값을 지정 안 해주면, 자동으로 null값이 들어가게 됨
INSERT INTO emp(eno, ename, job, mgr, hdate, sal, comm, dno)
...
--그러나 트와이스 21byte를 저장하려고 했으므로 에러 발생함.
INSERT INTO emp(eno, ename, job, mgr, hdate, sal, comm, dno)
    VALUES('1907', '엑스디오디너리', '밴드', null, sysdate, '9000', 9999, '40');
...
INSERT INTO emp(eno, ename, hdate)
    VALUES('1911', '김나라', TO_DATE('2022-07-02:07:15:29', 'YYYY-MM-DD:HH24:MI:SS'));
...
INSERT INTO emp(eno, hdate)
    VALUES('3', TO_DATE('99', 'RR')); --RR은 현재년도에서 2099년이 가까운지, 1999년이 가까운지
    INSERT INTO emp(eno, hdate)
    VALUES('4', sysdate);
