...
// @Copoment 어노테이션을 읽어서 자동으로 객체 준비/저장
// Container 클래스
public class ApplicationContext {
	...
	public ApplicationContext(String propertiesPath) throws Exception{
		...
		printAllPrepareObject(); // 어쩐 객체가 준비되었는지 확인
	}
	
	private void printAllPrepareObject() {
	...
	private void prepareAnnotationObjects() throws Exception{
		Reflections reflector = new Reflection("");
		
		Set<Class<?>> list = reflector.getTyprAnnotatedWith(Component.class);
		String key = null;
	...
	private void prepareObjects(Properties props) throws Exception{
		...
		for(Object item : props.keySet()) {
			...
			// 우리가 직접 new로 객체를 생성한다.
				...
				Class<?> classInfo = Class.forName(value); //
